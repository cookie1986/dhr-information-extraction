...
from dhrd.scraper.parsing.pagination import build_query_params, count_total_reports, calculate_total_pages
from dhrd.scraper.parsing.reports import parse_download_id, parse_dhr_reports
from dhrd.scraper.parsing.extraction import extract_report_data
    ...
    if check_file_exists(results_dir):
        ...
        for query in filters:
            ...
            for p_val in query["parameter_values"]:
                ...
                pages = []
                pages.append(get_html(url=url, timeout=3, params=params))
...
def extract_page_data(url: str, timeout: int = 3, results_dir: str = "data/raw/tags/"):
    ...
    pages.append(get_html(url=url, timeout=timeout, params={"pagination[pageSize]": PAGE_SIZE}))
    ...
    for page_num in range(1, total_pages):
        pages.append(get_html(url=url, timeout=timeout, params={"pagination[pageNumber]": page_num, "pagination[pageSize]": PAGE_SIZE}))
    ...
    extract_report_data(dhr_results=dhr_results, output_dir=results_dir)